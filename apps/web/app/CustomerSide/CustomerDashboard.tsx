...
export default function CustomerDashboard({ user }: { user?: { name: string; email: string } | null }) {
  ...
  return (
    <main className="flex-1 p-6 md:p-10 font-['Inter'] flex flex-col gap-10">
      ...
      {/* Exclusive Deals & Discounts Section (Visible if any product has an active discount) */}
      {(() => {
        const now = new Date();
        const isDeviceDiscountActive = (p: any) => {
          if (!p || !p.discount || p.discount <= 0) return false;
          if (p.discountStartDate && new Date(p.discountStartDate) > now) return false;
          if (p.discountEndDate && new Date(p.discountEndDate) < now) return false;
          return true;
        };

        const activeDiscountedProducts = products.filter(isDeviceDiscountActive);
