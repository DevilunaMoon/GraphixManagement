                ...
                <div className="bg-gray-50/50 border border-gray-100 rounded-3xl p-6 sm:p-8 flex flex-col gap-6 h-full shadow-[inset_0_2px_10px_rgba(0,0,0,0.01)]">
                  ...
                  <div className="flex justify-between items-start pb-6 border-b border-gray-200/60">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className={`inline-flex font-bold px-4 py-1.5 rounded-full text-sm mt-1 w-fit ${(device.status === 'Completed' || device.progress === '100%' || device.progress?.toLowerCase() === 'completed') ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                        {(device.status === 'Completed' || device.progress === '100%' || device.progress?.toLowerCase() === 'completed') ? 'Completed' : device.status}
                      </span>
                    </div>
